...
val_loader, in_loader = dl.MNIST(train=False, augm_flag=False, val_size=2000)
num_classes = 10
...
means = np.load('means.npy', allow_pickle=True).item()['MNIST']
means = [torch.tensor(m, device='cuda').float() for m in means]
stds = np.load('stds.npy', allow_pickle=True).item()['MNIST']
...
# Optimal hyperparams
hyperparams = {
    'LLL-RGPR-loglik': [3.3939e-08, 5.4485e-07, 1.1377e-07, 2.3509e-03],
    'LLL-RGPR-ood': [1.7384e-05, 1.6409e-06, 1.3555e-07, 2.5206e-03],
    'LLL': None,
    'BNO': None,
    'DE': None
}
...
def load_model(type):
    ...
    if type == 'BNO':
        ...
        state = torch.load(f'./pretrained_models/MNIST_blight_DSCS.pt')
        ...
        state_dicts = torch.load(f'./pretrained_models/MNIST_de.pt')
        for k in range(5):
            model[k].load_state_dict(state_dicts[k])
            model[k].eval()
    else:
        model.load_state_dict(torch.load(f'./pretrained_models/MNIST_plain.pt'))
        model.eval()
...
def evaluate(model_name, out_loader, out_targets, rgpr=False, loss_type='ood'):
    ...
    if model_name == 'LLL':
        model = model_
        model.eval()
        params = list(np.load(f'./pretrained_models/MNIST_plain_llla.npy', allow_pickle=True))
    else:
        model = model_
    ...
    print(f'[{model_str}] Acc: {acc:.3f}; NLL: {nll:.3f}; ECE: {ece:.3f}; Brier: {brier:.3f}')
...
for angle in range(0, 181, 15):
    ...
    out_loader = dl.RotatedMNIST(train=False, augm_flag=False, angle=angle)
...
# Save dict
dir_name = f'results/rotMNIST'

if not os.path.exists(dir_name):
    os.makedirs(dir_name)

with open(f'{dir_name}/tab_mmc_{args.randseed}.pkl', 'wb') as f:
    pickle.dump(tab_mmc, f)

with open(f'{dir_name}/tab_acc_{args.randseed}.pkl', 'wb') as f:
    pickle.dump(tab_acc, f)

with open(f'{dir_name}/tab_brier_{args.randseed}.pkl', 'wb') as f:
    pickle.dump(tab_brier, f)

with open(f'{dir_name}/tab_ece_{args.randseed}.pkl', 'wb') as f:
    pickle.dump(tab_ece, f)

with open(f'{dir_name}/tab_nll_{args.randseed}.pkl', 'wb') as f:
