...
def print_latex_ood_entries_aggregate2(d: dict):
    real_names = {
        'MNIST': 'MNIST',
        'CIFAR10': 'CIFAR-10',
        'FMNIST': 'F-MNIST',
    ...
    for k in d.keys():
        ...
        if compute_max:
            # Get min of MMC (first 5 components of the list) and max of AUR (the rests)
            lst = np.array(d[k]['mean'])[:5].round(1)
            min_mmc_idxs = np.argwhere(lst == np.min(lst)).flatten()

            lst = np.array(d[k]['mean'])[5:].round(1)
            max_aur_idxs = 5 + np.argwhere(lst == np.max(lst)).flatten()  # Start from idx 5
        ...
        for i, (m, s) in enumerate(zip(d[k]['mean'], d[k]['std'])):
            if m != -100:
                ...
                str = '-'
            entries.append(str)
        print(f'\\,\\,\\,\\,{real_names[out_dset]}' + ' & ' + ' & '.join(entries) + ' \\\\')
